...
# region agent log
DEBUG_LOG_PATH = r"/Volumes/Wangyd/砥脊科技_数据/003李文鹿OSCC/.cursor/debug-e4cbc6.log"
DEBUG_SESSION_ID = "e4cbc6"
...
debug_log("H1", "app.py:startup", "app import completed", {"python_runtime": "streamlit", "tf_version": tf.__version__, "shap_version": getattr(shap, "__version__", "unknown")})
# endregion
...
def calc_cut_off():
    ...
    debug_log(
        ...
        "app.py:calc_cut_off",
        "cutoff calculated",
        {"y_len": int(len(df_y)), "pred_len": int(len(df_pred_y)), "cutoff": float(cut_off)},
    )
    # endregion
    return cut_off
...
def load_model():
    ...
    try:
        ...
        debug_log(
            "H2",
            "app.py:load_model:error",
            "keras model load failed",
...
def determine_model_type():
    ...
    try:
        ...
        if unique_values <= 2 or df_y.dtype in ['object', 'category']:
            ...
            debug_log(
                "H3",
                "app.py:determine_model_type",
                "model type determined",
                {"model_type": model_type, "labels": [str(x) for x in labels]},
            )
            # endregion
...
if st.button("开始分析计算", key="calculate"):
    ...
    debug_log(
        "H4",
        "app.py:predict:before",
        "prediction starting",
        {"input_columns": list(input_df.columns), "input_shape": list(input_df.shape)},
    )
    # endregion
    try:
    ...
    try:
        ...
        debug_log(
            ...
            "app.py:shap",
            "shap generated",
            {"shap_shape": list(np.array(shap_values).shape), "base_value": base_value},
        )
        # endregion
    except Exception as e:
        # region agent log
    ...
    with tab2:
        ...
        try:
            ...
            debug_log(
                "H6",
                "app.py:decision_plot",
                "decision plot rendered",
            ...
            debug_log(
                "H6",
                "app.py:decision_plot:error",
                "decision plot failed",
                {"error_type": type(e).__name__, "error": str(e)},
            )
            # endregion
            raise
